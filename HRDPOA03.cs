...
namespace hrdNet
{
    ...
    class HRDPOA03
    {
        public void getHRDPOA03_01(string date)
        {
            try
            {
                string url = "http://www.hrd.go.kr/jsp/HRDP/HRDPO00/HRDPOA62/HRDPOA62_1.jsp?"; // Service Key
                ...
                {
                    Console.WriteLine("기업훈련과정 : " + i + "번째");
                    string param = "http://www.hrd.go.kr/jsp/HRDP/HRDPO00/HRDPOA62/HRDPOA62_1.jsp?"; // Service Key
                    ...
                    using (HttpWebResponse response = request2.GetResponse() as HttpWebResponse)
                    {
                        ...
                        for (int j = 0; j < itemList.ToList().Count; j++)
                        {
                            ...
                            sb.Append(" EXEC PROC_hrdNet_enterprise ");
        ...
        public void getHRDPOA03_02()
        {
            DataSet ds = Program.selectDS("select trprId,trprDegr,trainstCstId from HRDPOA03_01");

            Console.WriteLine("HRDPOA03_02 시작");
            int time = 0;
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                string url = "http://www.hrd.go.kr/jsp/HRDP/HRDPO00/HRDPOA62/HRDPOA62_2.jsp?authKey=ePJ32XXfC33CWJO40X7UsBa3MPIA4j5J&returnType=XML&outType=2" +
                    "&srchTrprId=" + dr["trprId"] +
                    "&srchTrprDegr=" + dr["trprDegr"] +
                    "&srchTorgId=" + dr["trainstCstId"];

                hrdNet_Detail detail = new hrdNet_Detail();
                detail.gethrdNet_Detail(url, "HRDPOA03");
            }

            Console.WriteLine("HRDPOA03_02 종료");
        }
    }
