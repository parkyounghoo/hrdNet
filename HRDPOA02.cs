...
namespace hrdNet
{
    ...
    class HRDPOA02
    {
        public void getHRDPOA02_01()
        {
            try
            {
                string url = "http://www.hrd.go.kr/jsp/HRDP/HRDPO00/HRDPOA61/HRDPOA61_1.jsp?authKey=ePJ32XXfC33CWJO40X7UsBa3MPIA4j5J&returnType=XML&outType=1&pageNum=1&pageSize=100&srchTraStDt=20201101&srchTraEndDt=20201231&sort=ASC&sortCol=TOT_FXNUM";
                ...
                {
                    Console.WriteLine("HRDPOA02" + i + "번째");
                    string param = "http://www.hrd.go.kr/jsp/HRDP/HRDPO00/HRDPOA61/HRDPOA61_1.jsp?"; // Service Key
                    param += "authKey=ePJ32XXfC33CWJO40X7UsBa3MPIA4j5J";
                    param += "&returnType=XML";
                    param += "&outType=1";
                    param += "&pageNum=" + i;
                    param += "&pageSize=100";
                    param += "&srchTraStDt=20201101";
                    param += "&srchTraEndDt=20201231";
                    param += "&sort=ASC";
                    ...
                    using (HttpWebResponse response = request2.GetResponse() as HttpWebResponse)
                    {
                        ...
                        for (int j = 0; j < itemList.ToList().Count; j++)
                        {
                            ...
                            sb.Append(" insert into HRDPOA02_01 values(");
        ...
        public void getHRDPOA02_02()
        {
            DataSet ds = Program.selectDS("select a.trprId, a.trprDegr, a.trainstCstId  from HRDPOA02_01 a where trprId in (select trprId from [hrdNet_Detail] where hrdGubun = 'HRDPOA02' and trprId != '') and trprDegr not in (select trprDegr from [hrdNet_Detail] where trprId = a.trprId) union all select a.trprId, a.trprDegr, a.trainstCstId from HRDPOA02_01 a where trprId not in (select trprId from [hrdNet_Detail] where hrdGubun = 'HRDPOA02' and trprId != '')");

            Console.WriteLine("HRDPOA02_02 시작");
            int time = 0;
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                string url = "http://www.hrd.go.kr/jsp/HRDP/HRDPO00/HRDPOA61/HRDPOA61_2.jsp?authKey=ePJ32XXfC33CWJO40X7UsBa3MPIA4j5J&returnType=XML&outType=2" +
                    "&srchTrprId=" + dr["trprId"] +
                    "&srchTrprDegr=" + dr["trprDegr"] +
                    "&srchTorgId=" + dr["trainstCstId"];

                hrdNet_Detail detail = new hrdNet_Detail();
                detail.gethrdNet_Detail(url, "HRDPOA02");
            }

            Console.WriteLine("HRDPOA02_02 종료");
        }
    }
